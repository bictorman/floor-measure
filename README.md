...
- **Add element** menu (needs the scale set first): place a **Circle** or **Rectangle** object, or draw a **Door** / **Window** opening. A door/window is two jamb ticks with a gap between them that masks the wall it crosses (so the wall reads as broken) — but it never affects line lengths or computed areas. Draw it click-click like a line, then drag its endpoints to resize/rotate, drag its body to move, or edit its width in the sidebar
...
- Project files are plain JSON with the image embedded as a base64 data URL. No zip / no external library. Shape: `{ format: "floorplan-measure", version: 1, savedAt, state: { imageDataUrl, imageW, imageH, lines, shapes, openings, calibrationLineId, pixelsPerMeter, zoom, panX, panY, rotation, imageOpacity, imageScaleX, imageScaleY } }`. Base64 makes the file ~33% larger than the raw image, but the file is human-inspectable and the format is trivial to parse by other tools.
